...
class BaseModel:
    ...
    def calculate_bvalues(self, arconf):
        s=np.zeros((int(conf["bresolution"],1)))
        pi=np.zeros((int(conf["bresolution"]),int(conf["bresolution"])))
        s[0,:]=float(conf["mu"])/(1-float(conf["rho"]))-m*math.sqrt(pow(float(conf["sig"]),2)/(1-(pow(float(conf["rho"]),2))))
        s[int(conf["bresolution"])-1,:]=float(conf["mu"])/(1-float(conf["rho"]))+m*math.sqrt(pow(float(conf["sig"]),2)/(1-(pow(float(conf["rho"]),2))))
        ...
        for j in range(0,int(conf["bresolution"])-1):
            for k in range(0,int(conf["bresolution"])-1):
                if k==0:
                    pi[j][k]=0.5*special.erfc((-(s[0,:]-float(conf["mu"])-float(conf["rho"]*s[j,:])+step/2)/float(conf["sig"]))/math.sqrt(2))
                elif k==int(conf["bresolution"]):
                    pi[j][k]=1-(0.5*special.erfc((-(s[int(conf["bresolution"])-1,:]-float(conf["mu"])-float(conf["rho"]*s[j,:])+step/2)/float(conf["sig"]))/math.sqrt(2)))
                else:
                    pi[j][k]=0.5*special.erfc((-(s[k,:]-float(conf["mu"])-float(conf["rho"]*s[j,:])+step/2)/float(conf["sig"]))/math.sqrt(2))- /
                    0.5*special.erfc((-(s[k,:]-float(conf["mu"])-float(conf["rho"]*s[j,:])+step/2)/float(conf["sig"]))/math.sqrt(2))
